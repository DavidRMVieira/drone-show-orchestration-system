...
/**
 * Request to list all proposals for a customer.
 */
public class ListCustomerScheduledShowsRequest extends ListShowProposalsRequest {
    ...
    public ListCustomerScheduledShowsRequest(final ListScheduledShowsController controller, Authenticator authenticationService,
    ...
    public String execute() {
        ...
        // execution
        try {
            final Iterable<ShowProposalDTO> proposalsDTO = listScheduledShowsController.listScheduledShowsByCustomer();

            if (!proposalsDTO.iterator().hasNext()) {
                return buildBadRequest("No proposals found for the customer with email: " + representativeEmail);
            }
